...
function force = MstSDFromA(TdA,TnA,Gamma,Vel)
    ...
    force_n = sum(W.*(0.5 * (curlg .* curlg - dot(psi,psi,2)) .* dot(normals,vels,2)),1);
    force_t = sum(W.* curlg .* dot(cross(normals,psi,2),vels,2),1);
