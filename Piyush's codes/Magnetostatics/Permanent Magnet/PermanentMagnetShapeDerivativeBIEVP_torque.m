...
function sd = PermanentMagnetShapeDerivativeBIEVP_torque(Gamma,TnA,TdA,J,omega_src,mu,mu0,M,Xcg)
    %% BEM Spaces
    % Source mesh
...
%     permI_gpu,permJ_gpu); 
    ...
    ptxFilePath = 'PermanentMagnetShapeDerivativeBIEVP_torque_GPU_I.ptx';
    cuFilePath = 'PermanentMagnetShapeDerivativeBIEVP_torque_GPU_I.cu';
    kernel = parallel.gpu.CUDAKernel(ptxFilePath, cuFilePath);
    kernel.GridSize = gridDim;
    kernel.ThreadBlockSize = blockDim; 

    shapeDerivative_gpuI0 = gpuArray.zeros(Nfields,1);
    shapeDerivative_gpuI1 = gpuArray.zeros(Nfields,1);
    shapeDerivative_gpuI2 = gpuArray.zeros(Nfields,1);
    shapeDerivative_gpuI3 = gpuArray.zeros(Nfields,1);

    % Interaction = 0 computations
    [shapeDerivative_gpuI0 ]= feval(kernel,...
    RWG.ndof,RWG.ndof,bndmesh.nelt,bndmesh.nvtx,size(I0,1),...
    Nthreads,gpuArray(cast(I0-1,'uint16')),gpuArray(cast(J0-1,'uint16')),...
    W0_gpu,X0_gpu,size(X0,2),...
    shapeDerivative_gpuI0,...
    TdA_gpu, TnA_gpu,Mxn_coeffs_gpu,...
    mu,mu0,...
    Elements_gpu,Vertices_gpu,Normals_gpu,Areas_gpu,...
    elt2dof_gpu,elt2dof_gpu,...
    gpuArray(cast(permI0','int32')),gpuArray(cast(permJ0','int32')),...
    Xcg_gpu); 

    %Interaction = 1 computations
    [shapeDerivative_gpuI1 ]= feval(kernel,...
    RWG.ndof,RWG.ndof,bndmesh.nelt,bndmesh.nvtx,size(I1,1),...
    Nthreads,gpuArray(cast(I1-1,'uint16')),gpuArray(cast(J1-1,'uint16')),...
    W1_gpu,X1_gpu,size(X1,2),...
    shapeDerivative_gpuI1,...
    TdA_gpu, TnA_gpu,Mxn_coeffs_gpu,...
    mu,mu0,...
    Elements_gpu,Vertices_gpu,Normals_gpu,Areas_gpu,...
    elt2dof_gpu,elt2dof_gpu,...
    gpuArray(cast(permI1'-1,'int32')),gpuArray(cast(permJ1'-1,'int32')),...
    Xcg_gpu); 

    % Interaction = 2 computations
    [shapeDerivative_gpuI2 ]= feval(kernel,...
    RWG.ndof,RWG.ndof,bndmesh.nelt,bndmesh.nvtx,size(I2,1),...
    Nthreads,gpuArray(cast(I2-1,'uint16')),gpuArray(cast(J2-1,'uint16')),...
    W2_gpu,X2_gpu,size(X2,2),...
    shapeDerivative_gpuI2,...
    TdA_gpu, TnA_gpu,Mxn_coeffs_gpu,...
    mu,mu0,...
    Elements_gpu,Vertices_gpu,Normals_gpu,Areas_gpu,...
    elt2dof_gpu,elt2dof_gpu,...
    gpuArray(cast(permI2'-1,'int32')),gpuArray(cast(permJ2'-1,'int32')),...
    Xcg_gpu); 

    % Interaction = 3 computations
    [shapeDerivative_gpuI3 ]= feval(kernel,...
    RWG.ndof,RWG.ndof,bndmesh.nelt,bndmesh.nvtx,size(I3,1),...
    Nthreads,gpuArray(cast(I3-1,'uint16')),gpuArray(cast(J3-1,'uint16')),...
    W3_gpu,X3_gpu,size(X3,2),...
    shapeDerivative_gpuI3,...
    TdA_gpu, TnA_gpu,Mxn_coeffs_gpu,...
    mu,mu0,...
    Elements_gpu,Vertices_gpu,Normals_gpu,Areas_gpu,...
    elt2dof_gpu,elt2dof_gpu,...
    gpuArray(cast(permI3','int32')),gpuArray(cast(permJ3','int32')),...
    Xcg_gpu); 

    shapeDerivative_gpu = shapeDerivative_gpuI3 + shapeDerivative_gpuI2 + shapeDerivative_gpuI1 + shapeDerivative_gpuI0;
