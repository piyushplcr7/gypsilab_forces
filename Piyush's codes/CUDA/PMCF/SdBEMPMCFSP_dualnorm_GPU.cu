...
__global__ void computeShapeDerivative(int TrialDim, int TestDim, int NTriangles, int NVertices, int NInteractions,
                                       ...
                                       double mu0,
                                       const double *Tdu, const double *Tnu, const double *Mdotn_coeffs,
    ...
    for (int idx = 0; idx < InteractionsPerThread; ++idx)
    {
        ...
        for (int QudPt = 0; QudPt < NQudPts; ++QudPt)
        {
            ...
            for (int fieldIdx = 0; fieldIdx < Nabc_alpha; ++fieldIdx)
            {
                ...
                localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -mu0 / 2 * 2 * Tnu[i] * Local_kerneloldmat_P0_P0 * Tnu[j]; // dbv_ds
                // atomicAdd(dbv_ds, -mu0 / 2 * (1 + mu0 / mu) * Tnu[i] * Local_kerneloldmat_P0_P0 * Tnu[j]);                                                                  // dbv_ds
                localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -mu0 * Tnu[i] * Local_kerneloldmat_P0_P0 * Mdotn_coeffs[j];              // l1
                localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -mu0 / 2 * Mdotn_coeffs[i] * Local_kerneloldmat_P0_P0 * Mdotn_coeffs[j]; // r2
                ...
                for (int jj = 0; jj < 3; ++jj)
                {
                    ...
                    localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -mu0 * Mdotn_coeffs[i] * (LocalMatrix_kernelintegrablemat_jj - LocalMatrix_combkernelmat_jj) * Tdu[EltJ[jj]]; // l23
                    ...
                    for (int ii = 0; ii < 3; ++ii)
                    {
                        ...
                        localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += mu0 / 2 * 2 * Tdu[EltI[ii]] * (LocalMatrix_kerneloldmat_nxgradP1_nxgradP1_ii_jj + 2 * LocalMatrix_SL_Dvelnxgrad_nxgrad_ii_jj) * Tdu[EltJ[jj]]; // dbw_ds
