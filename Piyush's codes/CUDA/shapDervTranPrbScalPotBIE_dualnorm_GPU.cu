    ...
    for (int idx = 0; idx < InteractionsPerThread; ++idx)
    {
        ...
        for (int QudPt = 0; QudPt < NQudPts; ++QudPt)
        {
            ...
            for (int fieldIdx = 0; fieldIdx < Nabc_alpha; ++fieldIdx)
            {
                ...
                localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -mu0 / mu * (mu0 - mu) * Tnu[i] * Local_kerneloldmat_P0_P0 * HJn_coeffs[j];                   // dl1/ds kerneloldmat_p0_p0 part
                localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += -(mu0 - mu) * (mu0 - mu) / 2 / mu * HJn_coeffs[i] * Local_kerneloldmat_P0_P0 * HJn_coeffs[j]; // remterms
                ...
                for (int jj = 0; jj < 3; ++jj)
                {
                    ...
                    localShapeDerivatives[fieldIdx + threadIdx.x * Nabc_alpha] += (mu0 - mu) * HJn_coeffs[i] * (LocalMatrix_kernelintegrablemat_jj - LocalMatrix_combkernelmat_jj) * Tdu[EltJ[jj]]; // dl3/ds
