    ...
    for (int idx = 0; idx < InteractionsPerThread; ++idx)
    {
        ...
        for (int QudPt = 0; QudPt < NQudPts; ++QudPt)
        {
            ...
            for (int fieldIdx = 0; fieldIdx < Nabc_alpha; ++fieldIdx)
            {
                ...
                //atomicAdd(dbv_ds, Tnu[i] * Local_kerneloldmat_P0_P0 * Tnu[j]);                                                                  // dbv_ds
                ...
                for (int jj = 0; jj < 3; ++jj)
                {
                    ...
                    //atomicAdd(dbk_ds, Tnu[i] * (LocalMatrix_kernelintegrablemat_jj - LocalMatrix_combkernelmat_jj) * Tdu[EltJ[jj]]);
                    ...
                    for (int ii = 0; ii < 3; ++ii)
                    {
                        ...
                        //atomicAdd(dbw_ds, Tdu[EltI[ii]] * (LocalMatrix_kerneloldmat_nxgradP1_nxgradP1_ii_jj + 2 * LocalMatrix_SL_Dvelnxgrad_nxgrad_ii_jj) * Tdu[EltJ[jj]]);
                    }
                }
            }
        }
