...
public:
  ...
  /*   matlab::data::TypedArray<int> _Elt2DofTest = std::move(inputs[22]);
    ...
    matlab::data::TypedArray<double> outputMatrixV = factory.createArray<double>(
        {NTriangles, NTriangles});

    matlab::data::TypedArray<double> outputMatrixK = factory.createArray<double>(
        {NTriangles, NVertices});

    matlab::data::TypedArray<double> outputMatrixW = factory.createArray<double>(
        {NVertices, NVertices});
    ...
    for (int i = 0; i < NVertices; ++i) {
      for (int j = 0; j < NVertices; ++j) {
        outputMatrixW[i][j] = Wmat(i, j);
      }
    }

    outputs[0] = outputMatrixV;
    outputs[1] = outputMatrixK;
    outputs[2] = outputMatrixW;
  }
};
