...
__device__ double KernelA1(int dir, Eigen::Vector3d Xcg, Eigen::Vector3d X, Eigen::Vector3d Y, Eigen::Vector3d YmX)
{
...
__device__ Eigen::Vector3d KernelC3(int dir, Eigen::Vector3d Xcg, Eigen::Vector3d X, Eigen::Vector3d Y, Eigen::Vector3d YmX)
{
    // return 1;
    double znorm = YmX.norm();
    return -3. / (4 * M_PI) * YmX * YmX.dot(Vel(dir, Xcg, Y) - Vel(dir, Xcg, X)) / (znorm * znorm * znorm * znorm * znorm) //
           + 1. / (4 * M_PI) * (Vel(dir, Xcg, Y) - Vel(dir, Xcg, X)) / (znorm * znorm * znorm);
}
...
__global__ void computeShapeDerivative(int TrialDim, int TestDim, int NTriangles, int NVertices, int NInteractions,
                                       int NThreads, const unsigned short int *I, const unsigned short int *J,
                                       const double *W, const double *X, int NQudPts,
                ...
                for (int ii = 0; ii < 3; ++ii)
                {
                    ...
                    for (int jj = 0; jj < 3; ++jj)
                    {
                        ...
                        double LocalMatrixA1_ii_jj = __ldg(&W[QudPt]) //
                                                     * KernelA1(fieldIdx, XCG, chi_tau, chi_t, chi_t - chi_tau) *
                                                     Psiy.dot(Psix);

                        // A2 with DVelRWG(y) . RWG(x)
                        double LocalMatrixA2_ii_jj = __ldg(&W[QudPt])                                                   //
                                                     * KernelA2(chi_tau, chi_t, chi_t - chi_tau) * (DVel(fieldIdx, XCG, //
                                                                                                         chi_t) *
                                                                                                    Psiy)
                                                                                                       .dot(Psix);
                        ...
                        double LocalMatrixC1_ii_jj = __ldg(&W[QudPt]) * //
                                                     (KernelC1(chi_tau, chi_t, chi_t - chi_tau).cross(DVel(fieldIdx, XCG, chi_t) * Psiy))
                                                         .dot(Psix);

                        // C3 with RWG X RWG
                        double LocalMatrixC3_ii_jj = __ldg(&W[QudPt])           //
                                                     * (KernelC3(fieldIdx, XCG, //
