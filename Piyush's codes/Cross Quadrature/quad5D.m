...
for i1 = 1:nq
   for i2 = 1:nq
      for i3 = 1:nq
         for i4 = 1:nq
             for i5 = 1:nq
              X(count, :) = x([i1 i2 i3 i4 i5])';
              W(count)    = prod(w([i1 i2 i3 i4 i5]));
              count = count + 1;
             end
         end
      end
...
[Xss{1}, Wss{1}] = quad1_cross(X, W); % common face

[Xss{2}, Wss{2}] = quad2_cross(X, W); % common edge

[Xss{3}, Wss{3}] = quad3_cross(X, W); % common vertex

[Xss{4}, Wss{4}] = quad4_cross(X, W); % far away
